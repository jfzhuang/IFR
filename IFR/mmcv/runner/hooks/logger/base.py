...
class LoggerHook(Hook):
    ...
    def get_loggable_tags(
        ...
        for var, val in runner.log_buffer.output.items():
            ...
            if 'train/threshold' in var:
                var = var.replace('train/', '')
            if 'train/class_acc' in var:
                var = var.replace('train/', '')
            if 'train/class_select_ratio' in var:
                var = var.replace('train/', '')
            if 'train/class_recall_ratio' in var:
                var = var.replace('train/', '')
            if 'train/IoU' in var:
                var = var.replace('train/IoU.', 'val_IoU/')
            if 'train/Acc' in var:
                var = var.replace('train/Acc.', 'val_Acc/')
            tags[var] = val
