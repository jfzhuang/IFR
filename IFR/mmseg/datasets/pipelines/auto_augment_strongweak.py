...
@PIPELINES.register_module()
class RandAugment_StrongWeak(object):
    ...
    def _check_policies(self, policies):
        for policy in policies:
            ...
            if magnitude_key is not None:
                assert 'magnitude_range' in policy, f'RandAugment_StrongWeak policy {type_name} needs `magnitude_range`.'
                magnitude_range = policy['magnitude_range']
                assert isinstance(magnitude_range, Sequence) and len(magnitude_range) == 2, (
                    f'`magnitude_range` of RandAugment_StrongWeak policy {type_name} ' f'should be a Sequence with two numbers.'
                )
