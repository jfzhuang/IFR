...
class SemiBaseSegmentor(BaseSegmentor):
    def train_step(self, data_batch, optimizer, iter, **kwargs):
        losses = self(**data_batch, iter=iter)
        loss, log_vars = self._parse_losses(losses)
    ...
    @auto_fp16(apply_to=('data_sup', ))
    def forward(self, data_sup=None, img_metas=None, return_loss=True, iter=None, **kwargs):
        """Calls either :func:`forward_train` or :func:`forward_test` depending
        on whether ``return_loss`` is ``True``.
        ...
        """
        if return_loss:
            return self.forward_train(data_sup, img_metas, iter=iter, **kwargs)
        else:
